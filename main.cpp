...
int main() {
    ...
    */
    ...
    waitKey(0);
    /*
    Mat result(1500,1500,CV_8UC3);

    Mat img = processRawImage(30);
...
Mat segmentThresh(Mat src){
    ...
    threshold(gray, bw, 150, 255, CV_THRESH_BINARY_INV | CV_THRESH_OTSU); // INV: invert for findcontours to work properly
    ...
    //Add padding for morphology to work correctly:
...
Mat segmentCanny(Mat src){
    ...
    morphologyEx(bw, bw, MORPH_CLOSE, element, Point(-1,-1), 2);
   // imshow("canny",bw);
